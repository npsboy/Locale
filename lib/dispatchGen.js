...
async function generateAndStore(dateKey) {
  ...
  for (const priorPool of pools) {
    const unused = priorPool.find(item => !shownLinks.has(item.link));
    if (!unused) continue;
    const reused = { ...unused, reused: true };
    await kv.set(dispatchKey(dateKey), JSON.stringify(reused), { ex: DISPATCH_TTL_SECONDS });
    return reused;
  }

  throw new Error(`No dispatch available for ${dateKey} and no unseen story left in the reserve`);
}
