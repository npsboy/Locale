...
// so findDispatch() is bounded — it tries a capped number of candidate
...
// Tries up to `maxCandidates` shuffled headlines, stopping at the first one
// that both geocodes and yields a full-length article body. Bounded (rather
// than looping over the whole feed) so this stays inside a serverless
// function's execution time limit — see the maxDuration/maxCandidates
// tradeoff noted in vercel.json.
async function findDispatch({ maxCandidates = 8, articleTimeoutMs = 5000 } = {}) {
