...
async function buildDispatch(candidate, articleTimeoutMs) {
  ...
  return { excerpt, place: geo.place, aliasPlace, lat: geo.lat, lng: geo.lng, source: item._source, link };
}
...
async function findDispatches({
  ...
  const located = geocodeHeadlines(shuffle(items).slice(0, MAX_TITLE_SCAN), excludeLinks);
  console.log(`[newswire] ${items.length} items, ${located.length} geocodable non-Delhi headlines`);
