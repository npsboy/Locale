...
// Long enough that a GeoNames outage almost never coincides with a rebuild:
// the data barely changes, and every day this cache is warm is a day the
// generator doesn't depend on geonames.org being up at all.
...
// The rebuild only happens when the cache is cold, so it gets a few tries
// before giving up — a transient blip there would otherwise take down a
// whole day's generation.
async function withRetries(label, fn) {
  ...
  for (let attempt = 1; attempt <= FETCH_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = e;
      console.error(`[gazetteer] ${label} attempt ${attempt}/${FETCH_ATTEMPTS} failed:`, e.message);
      if (attempt < FETCH_ATTEMPTS) await new Promise(r => setTimeout(r, attempt * 1000));
    }
  }
  throw lastError;
}
